...
int main()
{
	...
	Camera3D camera(glm::vec3(0.0f, 0.0f, 0.0f), 80, (float) screen->SCR_WIDTH / (float) screen->SCR_HEIGHT, glm::vec2(0.0f, 0.0f), 0.1,  200);
	...
	while (running)
	{
		...
		// Rendering
		screen->DrawModel(vertexShader, model, position, rotation, size, camera);
		screen->DrawBorder();
